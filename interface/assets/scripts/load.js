...
window.load = function() {
    ...
    if (!(typeof window.history.state == "object" && window.history.state != null)) {
        ...
        window.history.replaceState(defaultStateObject(), "", window.location.href);
...
function showPageMessage(title, message) {
    ...
    // Update the top bar
    topBar.dataset.allowFloat = allowSearchBarFloat = false;
...
// Generate the default state object
...
// Generate an updated state object
    ...
    redirect(url, changeURL = true) {
        ...
        if (pageFlags.canUnload) {
            ...
            if (changeURL) {
                ...
                window.history.pushState(defaultStateObject(), "", url);
